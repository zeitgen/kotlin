...
object FirThrowableSubclassChecker : FirBasicDeclarationChecker() {
    override fun check(declaration: FirDeclaration, context: CheckerContext, reporter: DiagnosticReporter) {
        if (declaration !is FirClass<*> || !declaration.hasThrowableSupertype(context))
            return

        if (declaration.typeParameters.isNotEmpty()) {
            reporter.reportGenericThrowableSubclass(declaration.typeParameters.firstOrNull()?.source)

            if (declaration.isInnerOrAnonymousObject()) {
                reporter.reportInnerClassOfGenericThrowableSubclass(declaration.source)
            }
        } else if (declaration.hasGenericInVisibility(context)) {
            reporter.reportInnerClassOfGenericThrowableSubclass(declaration.source)
        }
    }
    ...
    private fun FirClass<*>.isInnerOrAnonymousObject() =
        (this as? FirRegularClass)?.isInner == true || this is FirAnonymousObject

    private fun FirClass<*>.hasGenericInVisibility(context: CheckerContext) =

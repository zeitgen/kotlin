...
import org.jetbrains.kotlin.ir.backend.js.utils.addFunctionCall
...
class PropertyLazyInitLowering(
    ...
    override fun lower(irBody: IrBody, container: IrDeclaration) {
        ...
        when (container) {
            is IrSimpleFunction ->
                irBody.addFunctionCall(initializationCall, container)
            is IrField -> {
                container
                    ...
                    ?.filterNotNull()
                    ?.forEach {
                        irBody.addFunctionCall(initializationCall, it)
                    }
            }
        }
    }

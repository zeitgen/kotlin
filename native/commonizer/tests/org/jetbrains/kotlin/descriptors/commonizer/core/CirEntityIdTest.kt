...
class CirEntityIdTest {
    ...
    fun createFromClass() {
        listOf(
            Outer::class to listOf(CirEntityIdTest::class, Outer::class),
            Outer.Nested1::class to listOf(CirEntityIdTest::class, Outer::class, Outer.Nested1::class),
            Outer.Nested1.Nested2::class to listOf(CirEntityIdTest::class, Outer::class, Outer.Nested1::class, Outer.Nested1.Nested2::class)
        ).forEach { (clazz, nestingChain) ->
            val first = CirEntityId.create(clazz)
            val second = CirEntityId.create(clazz)
            assertSame(first, second)

            val rawPackageName = clazz.java.`package`.name
            assertEquals(rawPackageName, first.packageName.toString())

            val nestingChainSegments = nestingChain.map { CirName.create(it.simpleName!!) }.toTypedArray()
            assertTrue(nestingChainSegments.contentEquals(first.relativeNameSegments))
        }
    }

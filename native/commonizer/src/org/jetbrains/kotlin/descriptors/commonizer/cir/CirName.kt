...
class CirEntityId private constructor(val packageName: CirPackageName, val relativeNameSegments: Array<CirName>) {
    ...
    companion object {
        ...
        fun create(clazz: KClass<*>): CirEntityId = with(clazz.java) {
            val rawPackageName = `package`?.name ?: error("No package information for class $clazz")
            val rawRelativeName = canonicalName?.substringAfter(rawPackageName)?.removePrefix(".")
                ?: error("No canonical name for class $clazz")

            val packageName = CirPackageName.create(rawPackageName)
            val relativeNameSegments = splitComplexNameToArray(rawRelativeName, ".", CirName::create)

            create(packageName, relativeNameSegments)
        }
